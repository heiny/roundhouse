        ...
        public override void initialize_connections(ConfigurationPropertyHolder configuration_property_holder)
        {
            if (!string.IsNullOrEmpty(connection_string))
            {
                ...
                foreach (string part in parts)
                {
                    ...
                    if (string.IsNullOrEmpty(database_name) && (part.to_lower().Contains("port")))
                    {
                        port = part.Substring(part.IndexOf("=") + 1);
                    }
                }
            }
...
@"SET @had := (SELECT COUNT(1) FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = '{0}');
CREATE DATABASE IF NOT EXISTS {0};
SET @has := (SELECT COUNT(1) FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = '{0}');
SELECT (@had = 0 AND @has = 1) as created;"
            , database_name);
        }
